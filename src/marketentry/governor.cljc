(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Spain-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Spain-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Spain
                                       ...
                                       Grounded in EU establishment or
                                       authorized representative for many
                                       procedures (contracting authorities /
                                       PLACSP).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NIF/統一資格/PLACSP登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Spain public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はスペイン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- nif-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-nif? true`, INDEPENDENTLY check
  `:nif-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-nif? e))
        ...
        [{:rule :nif-unverified
          :detail (str subject " はNIF確認を要するが未確認 -- 提出提案は進められない")}]))))
