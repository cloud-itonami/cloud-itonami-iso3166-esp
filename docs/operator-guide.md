...
2. Register the client's intake: business type, target public function,
   prior filing history in Spain if any.
3. Run the advisor in read-only mode against the Plataforma de Contratación del Sector Público (PLACSP), Spain's official centralized public procurement platform.
...
- named referral relationship with Spanish-licensed counsel or a
...
a working referral relationship with Spanish-licensed counsel or a
registered agent for whatever licensed representation the law of
Spain requires for actual public-procurement filings.
