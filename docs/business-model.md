# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Spain

## Classification

- Repository: `cloud-itonami-iso3166-esp`
- ISO 3166: `ESP` (Spain)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Spanish
  public contract
...
- registration walkthrough for the Plataforma de Contratación del Sector Público (PLACSP), Spain's official centralized public procurement platform
- business/tax registration checklist: a Registro Mercantil (Commercial Registry) entry plus an NIF (Número de Identificación Fiscal); VAT (IVA) registration with the Agencia Tributaria
- local-content / preferential-procurement navigation: EU-wide open tendering above EU thresholds (no national-content quota as an EU member state), but Spanish-language submission and a domestic/EU address for service are typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Spanish-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Spain).
