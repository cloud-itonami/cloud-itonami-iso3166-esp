  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems ES"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems ES" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest nif-unverified-is-held-and-unoverridable
  (testing "unverified corporate number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:nif-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
